...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16):
    ...
    px, py = np.linspace(0, 1, 10000), []  # for plotting
    ap, p, r = np.zeros((nc, tp.shape[1])), np.zeros((nc, 10000)), np.zeros((nc, 10000))
    ...
    fn = (nt-tp).round() # false negative
    
    return tp, fp, fn, p, r, f1, ap, unique_classes.astype('int32')
...
def bbox_iou2(box1, box2, x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, EIoU=False, alpha=3, eps=1e-9):
...
def bbox_iou(box1, box2, x1y1x2y2=False, GIoU=False, DIoU=True, CIoU=False, eps=1e-7 , diou_beta1=1.0):
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU :  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            return iou - (rho2 / c2) ** diou_beta1  # DIoU iou - rho2 / c2  
        c_area = cw * ch + eps  # convex area
        return iou - (c_area - union) / c_area  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    return iou


def bbox_iou3(box1, box2, x1y1x2y2=False, GIoU=False, DIoU=True, CIoU=False, eps=1e-7 , diou_beta1=1.0):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    # box2 = box2.T # 요게 없는 버전이 bbox_iou3
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    df = pd.DataFrame(px)
    df2 = pd.DataFrame(py.mean(1))
    df.to_csv('/home/pms5343/python/yolov5-original/runs/sample.csv', index=False)
    df2.to_csv('/home/pms5343/python/yolov5-original/runs/sample2.csv', index=False)
