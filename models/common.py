...
import utils.softpool as sf
import utils.medianpool as mp
...
class SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.m = sf.SoftPooling2D(kernel_size=k, stride=1, padding=k // 2)
...
class TripletAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.no_spatial:
            x_out = self.hw(x)
            x_out = 1 / 3 * (1*x_out + 1*x_out11 + 1*x_out21) # alpha*x_out + beta*x_out11 + gamma*x_out21
        else:
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        #self.pool_h = sf.AdaptiveSoftPoolxd((None, 1))
        #self.pool_w = sf.AdaptiveSoftPoolxd((1, None))
...
class AdaptiveSoftPoolxd:
    ...
    def forward(self,x):
        ...
        # Apply pooling
        x = sf.soft_pool2d(x,kernel_size=kernel,stride=stride)
        return x
